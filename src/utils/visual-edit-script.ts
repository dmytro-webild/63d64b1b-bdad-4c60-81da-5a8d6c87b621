...
(function() {
  ...
  const makeEditable = (element, clickEvent) => {
    ...
    // been toggled off meanwhile (fast double-clicks, scroll-deselect). Going
    // editable then would set isEditing=true with a stale/null selectedElement,
    // wedging the editor (clicks eaten, blur/Escape guards never match).
